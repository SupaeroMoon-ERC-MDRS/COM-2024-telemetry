...
import 'package:unittests/ser.dart';
import 'package:unittests/des.dart';
...
abstract class FileSystem{
  ...
  static void setCurrentDirectory(final String dir){
  ...
  static Future<void> trySaveMapToLocalAsync(final String path, final String filename, final Map jsonEncodeable, {final bool withIndent = false}) async {
    ...
    if(withIndent){
      await access.writeFrom(Ser.prettyJsonToBytes(jsonEncodeable));
    }
    else{
      await access.writeFrom(Ser.jsonToBytes(jsonEncodeable));
    }
    await access.close();
  }
  ...
  static void trySaveMapToLocalSync(final String path, final String filename, final Map jsonEncodeable, {final bool withIndent = false}){
    ...
    if(withIndent){
      access.writeFromSync(Ser.prettyJsonToBytes(jsonEncodeable));
    }
    else{
      access.writeFromSync(Ser.jsonToBytes(jsonEncodeable));
    }
    access.closeSync();
  }
  ...
  static Future<Map> tryLoadMapFromLocalAsync(final String path, final String filename, {final bool deleteWhenDone = false}) async {
    ...
    try{
      return Des.jsonFromBytes(buffer);
    }catch(ex){
